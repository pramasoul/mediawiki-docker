...
if [ -z "$MEDIAWIKI_DB_PASSWORD" -a -r "$MEDIAWIKI_DB_PASSWORD_FILE" ]; then
	MEDIAWIKI_DB_PASSWORD=$(< $MEDIAWIKI_DB_PASSWORD_FILE)
	echo >&2 'info: set MEDIAWIKI_DB_PASSWORD from MEDIAWIKI_DB_PASSWORD_FILE'
	export MEDIAWIKI_DB_PASSWORD
fi

if [ -r "$MEDIAWIKI_ADMIN_PASS_FILE" ]; then
	MEDIAWIKI_ADMIN_PASS=$(< $MEDIAWIKI_ADMIN_PASS_FILE)
	echo >&2 'info: set MEDIAWIKI_ADMIN_PASS from MEDIAWIKI_ADMIN_PASS_FILE'
	export MEDIAWIKI_ADMIN_PASS
fi
...
if (getenv('MEDIAWIKI_DB_TYPE') == 'mysql') {
	...
	$mysql = new mysqli(getenv('MEDIAWIKI_DB_HOST'), getenv('MEDIAWIKI_DB_USER'), getenv('MEDIAWIKI_DB_PASSWORD'), getenv('MEDIAWIKI_DB_NAME'), (int) getenv('MEDIAWIKI_DB_PORT'));
